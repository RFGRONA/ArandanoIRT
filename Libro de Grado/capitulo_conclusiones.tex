...
\begin{itemize}
    ...
    \item \textbf{Detección temprana y sensibilidad:} El estudio experimental (PoC) evidenció que el sistema validado es suficientemente sensible para registrar los cambios en la temperatura foliar ($T_c$, $\Delta T$) y calcular el CWSI en respuesta a la restricción hídrica. La significancia estadística de la prueba T de Welch ($p = 0,0315$), ajustada mediante el tamaño muestral efectivo para corregir la autocorrelación temporal, constituye una prueba empírica confiable de que el sistema detecta la respuesta fisiológica. Asimismo, la robusta correlación inversa observada entre el VPD y el $\Delta T$ ($r \approx -0,88$) valida que el sensor MLX90640 traza con precisión la dinámica transpiratoria del cultivo bajo demandas atmosféricas variables.
\end{itemize}
...
Finalmente, durante la validación experimental (PoC), el sistema fue puesto a prueba monitoreando un individuo de arándano bajo condiciones controladas. Los resultados, sometidos a un riguroso análisis inferencial, revelaron una alta sensibilidad del sistema para capturar la respuesta fisiológica de la planta. El incremento del gradiente térmico ($\Delta T$) y la elevación sostenida del CWSI evidenciaron el cierre estomático frente a la sequía. La validez de estos hallazgos fue confirmada mediante una prueba de diferencia de medias que, tras penalizar la dependencia temporal de los datos, arrojó una significancia estadística de $p < 0,05$. Complementariamente, se obtuvo un coeficiente de correlación de Pearson de $-0,88$ entre el VPD y el $\Delta T$, demostrando que la matriz térmica de bajo costo se acopla a la perfección a la biofísica del cultivo. Todo este fenómeno fue detectado y transmitido de manera remota a la nube antes de que la planta exhibiera síntomas generalizados de daño visual permanente.
