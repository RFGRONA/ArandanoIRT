...
\begin{itemize}
    ...
    \item \textbf{Reuniones de seguimiento:} Se realizaron reuniones semanales con la directora del proyecto para revisar los adelantos, presentar el trabajo completado en el \textit{Sprint} y ajustar las prioridades para el siguiente ciclo.
\end{itemize}
...
La arquitectura resultante se describe desde dos perspectivas complementarias: el diseño lógico, que aborda la organización interna del código y la aplicación del patrón Modelo-Vista-Controlador (MVC), utilizando capas de la arquitectura cebolla (Onion Architecture); y la arquitectura física, que detalla la distribución de los componentes de software en la infraestructura de despliegue. Finalmente, se presenta el stack (recursos) tecnológico que sustenta el sistema.
...
\begin{enumerate}
    \item \textbf{Servidor central (cloud VM)}: Se utiliza una Máquina Virtual (VM) alojada en el proveedor de infraestructura como servicio (IaaS), específicamente DigitalOcean. Este nodo ejecuta el sistema operativo \textbf{Ubuntu Server 24.04 LTS} y alberga la totalidad de la pila de servicios backend, orquestada mediante Docker y Docker Compose. Los componentes de software que se ejecutan en contenedores dentro de esta VM incluyen:
        ...
        \end{itemize}
    El servidor está protegido por firewalls a nivel de red (proveedor cloud) y a nivel de host (\texttt{ufw}), además de contar con \texttt{NGINX Proxy Manager} para la gestión de conexiones y puertos, además de prevención de intrusiones.
...
\begin{itemize}
    ...
    \item \textbf{Pila de observabilidad}: Para el monitoreo y diagnóstico del sistema, se implementó una solución basada en el stack de Grafana Labs. Aunque esta pila es más exigente en términos de recursos computacionales que opciones ligeras como Dozzle (limitada a visualización en tiempo real sin persistencia) o soluciones balanceadas como OpenObserve y Parseable, su elección se justifica por ser un estándar de industria en sistemas de alta criticidad. La solución se compone de:
        \begin{itemize}
            \item \textbf{Agregación de logs}: \textbf{Loki} centraliza y almacena de forma eficiente los logs generados por todos los contenedores.
            \item \textbf{Recolección de logs}: \textbf{Promtail} actúa como el agente encargado de recolectar, etiquetar y enviar los logs al servidor de Loki.
            \item \textbf{Visualización y alertas}: \textbf{Grafana} permite la consulta avanzada de logs mediante \textit{LogQL}, la creación de tableros de control y la configuración de alertas proactivas ante fallos críticos.
        \end{itemize}
...
\begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF01: Registro}
	\label{tab:rf01}
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF02: Inicio de sesión}
	\label{tab:rf02}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF03: CRUD cámara}
	\label{tab:rf03}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF04: CRUD persona}
	\label{tab:rf04}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF05: Módulo de mediciones}
	\label{tab:rf05}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF06: Módulo de procesamiento}
	\label{tab:rf06}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF07: Reportes}
	\label{tab:rf07}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF08: Notificaciones}
	\label{tab:rf08}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento Funcional RF09: Gestionar Observaciones}
	\label{tab:rf09}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento No Funcional RNF01: Seguridad}
	\label{tab:rnf01}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
  ...
  \begin{table}[H]
	\centering
	\caption{Requerimiento No Funcional RNF02: Copia de seguridad}
	\label{tab:rnf02}
	\begin{tabular}{@{}ll@{}} 
	  \toprule
...
Un caso de uso es una unidad coherente de funcionalidad externamente visible proporcionada por un clasificador (denominado sistema) y expresada mediante secuencias de mensajes intercambiados por el sistema y uno o más actores de la unidad del sistema \shortcite{Rumbaugh2007}. El propósito de un caso de uso es definir una pieza de comportamiento coherente sin revelar la estructura interna del sistema \shortcite{Rumbaugh2007}.
...
Un diagrama de secuencia muestra un conjunto de mensajes ordenados en una secuencia temporal \shortcite{Rumbaugh2007}. Cada rol se muestra como una línea de vida es decir, una línea vertical que representa al rol a lo largo del tiempo a través de la interacción completa \shortcite{Rumbaugh2007}. Los mensajes se muestran con flechas entre líneas de vida \shortcite{Rumbaugh2007}.
...
Una actividad muestra el flujo de control entre las actividades computacionales involucradas en la realización de un cálculo o un flujo de trabajo \shortcite{Rumbaugh2007}. Una acción es un paso computacional primitivo y un nodo de actividad es un grupo de acciones o subactividades \shortcite{Rumbaugh2007}. Una actividad describe tanto el cómputo secuencial como el concurrente \shortcite{Rumbaugh2007}.
...
Una clase es la descripción de un concepto del dominio de la aplicación o del dominio de la solución \shortcite{Rumbaugh2007}. Las clases son el centro alrededor del cual se organiza la vista de clases \shortcite{Rumbaugh2007}. La vista estática se muestra en los diagramas de clases \shortcite{Rumbaugh2007}.
...
La vista de despliegue muestra la disposición física de los nodos, que son recursos computacionales de tiempo de ejecución, como computadoras u otros dispositivos \shortcite{Rumbaugh2007}. Durante la ejecución, los nodos pueden contener artefactos, que son entidades físicas como archivos \shortcite{Rumbaugh2007}.
...
Para evaluar la calidad del código fuente desarrollado para el sistema \textit{Arandano IRT}, se utilizó la herramienta de análisis estático de código SonarQube (Community Edition). El análisis estático permite examinar el código sin ejecutarlo, identificando posibles errores de programación (bugs), vulnerabilidades de seguridad, duplicaciones y malas prácticas (code smells) que podrían afectar la fiabilidad, mantenibilidad y seguridad del software a largo plazo.

La Figura \ref{fig:sonarqube_results} muestra el resumen del análisis realizado por SonarQube sobre la rama principal (`main`) del repositorio de código del proyecto (\texttt{ArandanoIRTSoftware}).

\begin{figure}[H] 
    \centering
    \caption{Resultados del análisis estático con sonarqube para el proyecto arandano IRT.}
    \includegraphics[width=0.7\textwidth]{img/sonarqube.png}
    \label{fig:sonarqube_results}
\end{figure}

Los resultados clave obtenidos del análisis son los siguientes:

\begin{itemize}
    \item \textbf{Quality gate:} El proyecto superó exitosamente el "Quality Gate" predeterminado de SonarQube, indicado por el estado "Passed". Esto significa que, según los umbrales por defecto de la herramienta, la calidad general del código nuevo (en este caso, todo el código base) cumple con los mínimos aceptables.
    \item \textbf{Seguridad (security):} El análisis no detectó vulnerabilidades abiertas ni puntos críticos de seguridad (Security Hotspots), otorgando una calificación 'A' en esta categoría. Este es un resultado positivo, indicando una buena adherencia a prácticas de codificación segura básicas.
    \item \textbf{Fiabilidad (reliability):} Se identificaron 10 problemas abiertos relacionados con la fiabilidad, resultando en una calificación 'C'. Estos problemas típicamente señalan código que podría comportarse de manera inesperada o provocar errores en ciertas condiciones (ej. manejo inadecuado de nulos, posibles excepciones no controladas).
    \item \textbf{Mantenibilidad (maintainability):} Se reportaron 254 problemas abiertos clasificados como "code smells", pero el sistema obtuvo una calificación 'A' en mantenibilidad. Esto sugiere que, aunque existen numerosas áreas con código que podría refactorizarse para mejorar la legibilidad o la eficiencia según las reglas de SonarQube, la deuda técnica estimada asociada es baja, y la estructura general del código es considerada mantenible.
    \item \textbf{Duplicaciones (duplications):} El nivel de código duplicado se estimó en 1,3\% sobre aproximadamente 25,000 líneas de código analizadas. Este es un porcentaje relativamente bajo, indicando un uso adecuado de abstracciones y reutilización.
    \item \textbf{Cobertura de pruebas (coverage):} El análisis reportó una cobertura de código del \textbf{0,0\%}. Este resultado indica la ausencia de una suite de pruebas automatizadas (unitarias, de integración) que ejerciten el código fuente.
\end{itemize}

La falta de cobertura de pruebas automatizadas (0,0\%) es una limitación reconocida de la implementación actual. Dada la priorización del desarrollo de la funcionalidad central requerida por el MVP y la posterior implementación completa del sistema dentro de los plazos académicos establecidos, la creación de una suite exhaustiva de pruebas automatizadas fue diferida. Si bien se realizaron pruebas manuales funcionales durante las iteraciones de desarrollo para asegurar la correcta operación del sistema, la ausencia de pruebas automatizadas representa un componente de deuda técnica. Se recomienda la incorporación de estrategias de pruebas automatizadas (como pruebas unitarias para la lógica de negocio y pruebas de integración para los flujos clave) en futuras fases de mantenimiento o evolución del sistema, lo cual mejoraría significativamente la fiabilidad ante cambios y facilitaría la refactorización segura del código.

El análisis estático con SonarQube proporcionó una evaluación objetiva de la calidad del código, destacando fortalezas importantes en seguridad y un nivel aceptable de mantenibilidad y duplicación. Sin embargo, también evidenció áreas de mejora en fiabilidad y, de manera crítica, la necesidad de incorporar pruebas automatizadas como una práctica esencial para garantizar la robustez y facilitar el mantenimiento futuro del software \textit{Arandano IRT}.
...
El método UCP evalúa la complejidad del sistema mediante el cálculo secuencial de puntos no ajustados (basados en actores y casos de uso, $UUCP$), ajustados luego por factores técnicos ($TCF$) y ambientales ($ECF$). El resultado final ($UCP$) se multiplica por un factor de productividad (horas-persona por UCP) para obtener la estimación total de esfuerzo.
...
\section{Resultados y evaluación de la implementación del software}
\label{sec:resultados_implementacion}

La implementación del software \textit{Arandano IRT} culminó con el despliegue de un ecosistema digital funcional y resiliente, superando el objetivo de proveer una plataforma accesible para el monitoreo del estrés hídrico. Más allá de la entrega del código, el análisis de los resultados de esta fase permite evaluar la efectividad de las decisiones arquitectónicas y metodológicas adoptadas.

En primer lugar, la adopción de un enfoque iterativo basado en un Prototipo Mínimo Viable (MVP) operó como una estrategia crítica de mitigación de riesgos. Esta fase temprana permitió probar el canal de ingesta de datos de la API, validando la concurrencia entre PostgreSQL y ASP.NET Core antes de escalar el sistema. El resultado tangible de esta estrategia fue la recolección de un conjunto de datos inicial superior a 3.000 registros térmicos y ambientales, el cual se convirtió en el insumo fundamental para la viabilidad de la Prueba de Concepto (PoC) agronómica.

A nivel de diseño lógico, la implementación de una arquitectura monolítica modular estructurada bajo los principios de la Arquitectura Cebolla (Onion Architecture) demostró ser altamente efectiva. La separación estricta de responsabilidades, aislando el \textit{Dominio} de la \textit{Infraestructura}, previno el acoplamiento excesivo del código fuente. La efectividad de esta decisión se vio respaldada cuantitativamente por el análisis de SonarQube (Sección \ref{sec:calidad_codigo}), donde el sistema obtuvo una calificación \textit{A} en Mantenibilidad y Seguridad, evidenciando una base de código limpia y con un índice de duplicación marginal ($1,3\%$).

A nivel de infraestructura física, la decisión de migrar hacia una pila tecnológica auto-hospedada y contenerizada (Docker) se validó empíricamente tras un incidente de denegación de servicio indirecta causado por un proveedor externo (Cloudflare) durante el MVP. Al internalizar y orquestar componentes críticos como el proxy inverso (Caddy), el almacenamiento de objetos (MinIO) y la observabilidad (Loki/Grafana), la arquitectura final alcanzó una total autonomía operativa. Como resultado, el sistema en producción documentó un \textit{Uptime} (tiempo de actividad) del $100\%$ durante más de 100 días consecutivos de operación, garantizando la continuidad en la recolección de datos desde los dispositivos IoT.

En términos de alcance y gestión de proyecto, el software logró cubrir la totalidad de los 36 casos de uso funcionales estipulados (Sección \ref{sec:casos_uso}). La validación de la estimación de recursos mediante Puntos de Casos de Uso (UCP) arrojó un esfuerzo calculado de 3.345 horas-persona. Esta métrica no solo reflejó con realismo el esfuerzo invertido por el equipo de desarrollo durante los 10 meses del proyecto, sino que estableció una línea base confiable para la estimación de costos y tiempos en futuros mantenimientos o escalamientos comerciales.

En síntesis, la implementación validó que el ecosistema \textit{.NET 8} combinado con un despliegue contenerizado proporciona la robustez necesaria para soportar sistemas de agricultura de precisión. No obstante, el ciclo de vida de la ingeniería de software exige una evaluación crítica constante; por lo tanto, a continuación se detallan las concesiones técnicas asumidas durante esta fase y la proyección de la arquitectura hacia un modelo escalable.

\subsection{Limitaciones del software y la arquitectura actual}
\label{subsec:software_limitaciones}

A pesar del éxito en la implementación y la alta disponibilidad demostrada por el sistema, un análisis crítico de la arquitectura y el código fuente revela ciertas limitaciones técnicas que deben considerarse para futuras iteraciones o despliegues a mayor escala:

\begin{itemize}
    \item \textbf{Carencia de pruebas automatizadas (Deuda Técnica):} Como se evidenció en el análisis estático de código mediante SonarQube (Sección \ref{sec:calidad_codigo}), el proyecto presenta una cobertura de pruebas automatizadas del $0,0\%$. La validación del sistema dependió exclusivamente de pruebas manuales (QA) durante los \textit{Sprints}. Aunque funcional para una Prueba de Concepto (PoC), esta carencia representa una deuda técnica significativa que dificulta la refactorización segura y el mantenimiento a largo plazo.
    
    \item \textbf{Escalabilidad vertical de la arquitectura monolítica:} Si bien la arquitectura monolítica en ASP.NET Core 8 facilitó el desarrollo ágil para un equipo pequeño, este enfoque acopla estrechamente los módulos de procesamiento pesado (como el cálculo del CWSI y el manejo de matrices térmicas) con los módulos de presentación (interfaz de usuario). En un escenario de escalamiento masivo (cientos de hectáreas y miles de sensores transmitiendo en tiempo real), los picos de procesamiento de datos podrían degradar el tiempo de respuesta de la aplicación web.
    
    \item \textbf{Gestión de colas de mensajes y resiliencia de ingesta:} Actualmente, la API recibe los datos de los dispositivos IoT mediante solicitudes HTTPS síncronas. Aunque es funcional para la PoC, no se ha implementado un \textit{Message Broker} (como RabbitMQ o Kafka). En caso de una caída momentánea del servidor base de datos o un pico masivo de peticiones, las solicitudes de los ESP32-S3 podrían perderse si los dispositivos agotan su memoria local (SD) antes de que el servidor se recupere.
    
    \item \textbf{Acoplamiento de la segmentación térmica:} El procesamiento y segmentación de las imágenes térmicas para aislar la canopia foliar se diseñó de manera externa  y manual (procesamiento posterior) o acoplada al ciclo base. La falta de un microservicio de visión artificial dedicado limita la capacidad del sistema para aplicar algoritmos de \textit{Machine Learning} en tiempo real sobre las capturas entrantes antes de que sean persistidas.
\end{itemize}

\subsection{Trabajo futuro y evolución de la arquitectura}
\label{subsec:software_trabajo_futuro}

Para mitigar las limitaciones identificadas y preparar el sistema \textit{Arandano IRT} para un despliegue comercial o investigaciones a gran escala, se proponen las siguientes líneas de evolución técnica:

\begin{itemize}
    \item \textbf{Implementación de un entorno de \textit{Testing} continuo:} Es imperativo desarrollar una suite integral de pruebas unitarias (utilizando frameworks como xUnit o NUnit) focalizada en la capa de \textit{Domain} y \textit{Application}, garantizando que los cálculos críticos (como el CWSI o la validación de límites ambientales) operen correctamente. Asimismo, se deben integrar pruebas de integración para los \textit{Endpoints} de la API de dispositivos.
    
    \item \textbf{Migración hacia una arquitectura orientada a microservicios:} Para resolver los cuellos de botella de escalabilidad, se sugiere desacoplar el monolito separando las responsabilidades en microservicios independientes. Por ejemplo, un servicio dedicado exclusivamente a la ingesta de datos IoT (\textit{Ingestion API}), otro para el cálculo agronómico (\textit{Processing Service}), y otro para la interfaz de usuario (\textit{Web UI}). Esta separación permitiría escalar horizontalmente solo los servicios que lo requieran (ej. añadir más nodos de ingesta si se instalan más cámaras).
    
    \item \textbf{Integración de un \textit{Message Broker} para la ingesta de datos:} Incorporar un sistema de colas (ej. Apache Kafka o MQTT) entre la API de dispositivos y la base de datos PostgreSQL. Esto permitiría que la API reciba los \textit{payloads} JSON térmicos y ambientales de los ESP32-S3 de forma inmediata, encolándolos para su posterior procesamiento asíncrono. Esta estrategia de \textit{Fire-and-Forget} garantizaría que no se pierdan datos incluso bajo alta concurrencia.
    
    \item \textbf{Desarrollo de un microservicio de visión artificial (\textit{Computer Vision}):} Extraer la lógica de segmentación térmica a un servicio en Python utilizando librerías especializadas (como OpenCV o PyTorch). Este servicio podría ser consultado por la aplicación principal vía gRPC o API REST para aplicar segmentación inteligente (separando automáticamente el dosel del fondo) en el momento en que se recibe el termograma, automatizando la limpieza de datos antes de calcular el CWSI.
    
    \item \textbf{Transición a un esquema de base de datos de series temporales (TSDB):} Aunque PostgreSQL 16 es altamente capaz, el almacenamiento masivo de matrices térmicas (768 puntos de datos cada 20 minutos por planta) crecerá exponencialmente. Explorar la migración de las tablas \texttt{environmental\_readings} y \texttt{thermal\_captures} a extensiones como TimescaleDB, o bases de datos específicas como InfluxDB, optimizaría significativamente las consultas de los históricos y la generación de los reportes.
    
    \item \textbf{Modelo de despliegue de instancia dedicada (\textit{Single-Tenant}):} Desde la perspectiva de escalabilidad comercial y modelo de negocio, la arquitectura del sistema se proyecta hacia un esquema de \textit{instancia por finca} (modelo de despliegue de Inquilino Único (Single-Tenant) o Instancia Dedicada) en lugar de una macroplataforma centralizada (\textit{Multi-Tenant}). Esta decisión de segmentar el despliegue garantiza la soberanía y el aislamiento absoluto de los datos de cada productor, previniendo que incidentes de infraestructura o corrupción de datos en un entorno afecten al resto de los clientes, lo que otorga un (\textit{aislamiento de fallos}). Comercialmente, esta modularidad permite gestionar actualizaciones distribuidas y ofrecer esquemas de monetización flexibles, activando servicios \textit{premium} (como análisis predictivos con IA o almacenamiento extendido en MinIO) de forma selectiva, adaptándose a los Acuerdos de Nivel de Servicio (SLA) y la capacidad de inversión de cada productor agrícola.
